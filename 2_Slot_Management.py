...
with tabs[1]:
    ...
    with col_qr:
        ...
        url_base = st.text_input("URL del Portal", value="https://proyectodetitulo-slot-management-jvillalobosp2-gwoohdfkxqqvjru.streamlit.app/Portal_Transportistas", help="Link público del portal móvil para transportistas")
        ...
        st.image(byte_im, caption="Escanea: Portal de Agendamiento", use_container_width=True)
        
        st.markdown("---")
        # QR Adicional para el Dashboard
        dash_url = "https://proyectodetitulo-slot-management-jvillalobosp2-gwoohdfkxqqvjru.streamlit.app/"
        qr_d = qrcode.QRCode(version=1, box_size=10, border=4)
        qr_d.add_data(dash_url)
        qr_d.make(fit=True)
        img_d = qr_d.make_image(fill_color="black", back_color="white")
        buf_d = io.BytesIO()
        img_d.save(buf_d, format="PNG")
        st.image(buf_d.getvalue(), caption="Escanea: Ver Dashboard de Planta", use_container_width=True)
        
        st.download_button(
            label="Descargar QR Agendamiento",
            data=byte_im,
